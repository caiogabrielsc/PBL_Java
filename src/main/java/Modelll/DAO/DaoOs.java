...
public class DaoOs {
    ...
    public static boolean finalizeOsDaoFile(int id, double value) throws IOException, ClassNotFoundException {
        ...
        for (Os os : listFile) {
            if (os.getId() == id) {
                os.setStatus(2);  //aguardando pagamento
    ...
    public static boolean listOsFile() throws IOException, ClassNotFoundException {
        ...
        for (Os os : listFile) {
            System.out.println("---------------------------------------------------------------");
            System.out.println("idOs:" + os.getId() + " | cliente:" + os.client.getName() + " | status:" + os.getStatus() + " | atendente:" + os.getAttendant().getName());
        }
        System.out.println("---------------------------------------------------------------\n");
    ...
    public static boolean changeClientOsDaoFile(int id, int idClient) throws IOException, ClassNotFoundException {
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("os.bin"));
        List<Os> listFile = (List<Os>) ois.readObject();
        ois.close();
        ...
        for (Os os : listFile) {
            if (os.getId() == id) {
                os.setClient(DaoPessoa.returnClientByIdFile(idClient));
            }
        }
        ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("os.bin"));
        oos.writeObject(listFile);
    ...
    public static boolean changeDescriptionOsDaoFile(int id, String description) throws IOException, ClassNotFoundException {
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("os.bin"));
        List<Os> listFile = (List<Os>) ois.readObject();
        ois.close();
    ...
    public static Os returnOsById(int id) throws IOException, ClassNotFoundException {
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("os.bin"));
        List<Os> listFile = (List<Os>) ois.readObject();
        ois.close();
